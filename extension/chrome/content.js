// Content script to extract Slack tokens
(function() {
    'use strict';
    ...
    function isHackClubWorkspace() {
        ...
        if (window.location.hostname === HACK_CLUB_DOMAIN || 
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        if (request.action === 'getTokens') {
            extractTokens().then(tokens => {
                sendResponse({ tokens: tokens });
            }).catch(error => {
                console.error('Error extracting tokens:', error);
                sendResponse({ tokens: { isHackClub: false, xoxc: null, xoxd: null } });
            });
